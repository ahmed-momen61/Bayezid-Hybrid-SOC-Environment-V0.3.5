...
def compile_to_llvm_ir(python_code: str) -> str:
    # Convert Python to C using Cython, then to LLVM-IR via clang
...
async def llvm_mutate(req: Request):
    ...
    if is_windows:
        ...
        return {
            ...
            "elf_b64": base64.b64encode(mutated_code.encode()).decode(), # Return python string directly for fallback
            "seed": seed,
            "mode": "ast_fallback"
        }
    else:
        try:
            ...
            # If clang/cython fails on linux, still fallback
            mutated_code = mutate_ast_fallback(payload, seed)
            return {
                "mutated_ir_hash": hashlib.sha256(mutated_code.encode()).hexdigest(),

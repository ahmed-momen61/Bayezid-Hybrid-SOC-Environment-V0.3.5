...
class NetworkNode {
    constructor(ip, hostname = '', os = 'unknown', services = []) {
        ...
        this.os = os;
        this.services = services; // [{port, protocol, service, version}]
        this.subnet = ip.split('.').slice(0, 3).join('.') + '.0/24';
        this.risk = 0; // 0-100 risk score
        this.compromised = false;
        this.isolated = false;
        this.neighbors = new Map(); // ip -> {weight, latency, protocol}
        this.features = null; // GNN feature vector
...
class NetworkGNN {
    constructor() {
        this.nodes = new Map(); // ip -> NetworkNode
        this.edges = [];
        this.featureDim = 16; // Feature vector dimension
        this.propagationLayers = 3;
        this.lateralMovementThreshold = 0.7; // Risk threshold for pre-emptive isolation
    }
    ...
    initializeFeatures() {
        for (const [ip, node] of this.nodes) {
            ...
            features[0] = node.neighbors.size / Math.max(this.nodes.size, 1); // Degree centrality
            features[1] = node.services.length / 20; // Service density
            features[2] = node.compromised ? 1.0 : 0.0; // Compromise status
            features[3] = node.risk / 100; // Current risk score

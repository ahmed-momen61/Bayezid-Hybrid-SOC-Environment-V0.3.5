...
const authMiddleware = async(req, res, next) => {
    // Standardize path ignoring query params
    const routeKey = `${req.method}:${req.path.replace(/\/$/, '')}`;
    const requiredRole = ROUTE_POLICY[routeKey];
    if (!requiredRole) return next(); // public or unprotected
...
// ═══════════════════════════════════════════════════════════
// RULES OF ENGAGEMENT (RoE) — Cryptographic Authorization Gate
// ═══════════════════════════════════════════════════════════
...
const enforceRoE = async(req, res, next) => {
    ...
    try {
        ...
        // Check if token belongs to requesting operator (if JWT middleware passed)

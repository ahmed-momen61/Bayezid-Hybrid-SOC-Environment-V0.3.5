...
const HONEY_TOKENS = [
    ...
    "ROOT_SSH_KEY_B64=LS0tLS1CRUdJTiBPUEVOU1NIIFBSSVZBVEUgS0VZLS0tLS0=" // Fake base64 key
];
...
class LateralDefense {
    ...
    startEnvironmentMorphing() {
        ...
        this.morphingInterval = setInterval(() => {
            ...
            for (const [container, oldToken] of this.activeDecoys.entries()) {
                this.deployHoneyTokens(container); // redeploys a new one, overwriting the map
            }
        }, 60000);
    }
    ...
    async deployHoneyTokens(targetContainer) {
        ...
        try {
            // Simulate injecting this into the container's environment or filesystem
    ...
    monitorDecoyUsage(telepathyBroadcast) {
        // The Blue Team taps into the latent network to detect if the Red Team gossips their Honey Token
    ...
    async isolateNode(nodeName) {
         ...
         try {
             // Simulate network isolation (e.g., removing from docker network or iptables drop)
